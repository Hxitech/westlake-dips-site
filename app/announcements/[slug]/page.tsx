...
export default async function AnnouncementDetailPage({
  ...
  const { slug } = await params;
  const isMeetingNotice = slug === "2026-meeting-notice";
    ...
    <section className="container-shell pb-24 pt-16 sm:pb-28 sm:pt-20">
      <div className="max-w-5xl">
        ...
        <div className="mt-8 panel rounded-[2.25rem] p-7 sm:p-10">
          <div className="text-body-copy flex flex-wrap items-center gap-3 uppercase tracking-[0.16em] text-gray-400">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {related.map((item) => (
              <Link
                ...
                key={item.slug}
              >
                <div className="text-body-copy uppercase tracking-[0.16em] text-blue-600">
                  <T zh={item.category} en={item.categoryEn ?? item.category} />
                </div>
                <h2 className="text-body-copy mt-3 font-semibold leading-snug text-gray-900">
